...
def make_aware(dt):
    """Ensure datetime is timezone-aware, or None."""
    if dt is None:
        return None
...
def parse_bool(val):
    """Normalize boolean-like values to True, False, or None."""
    if val in [True, 1, '1', 'true', 'True', 'yes', 'Yes']:
        return True
    elif val in [False, 0, '0', 'false', 'False', 'no', 'No']:
        return False
    return None
...
def run_import_background(task_id, selected_ids):
    """
    Run import in background thread with full timezone and boolean normalization
    """
    try:
                    ...
                    with transaction.atomic():
                        recipient, created = Recipient.objects.update_or_create(
                            ...
                            defaults={
                                ...
                                'street2': sp.street2,
                                'zip': sp.zip,
                                'active': parse_bool(sp.active),
                                'is_company': parse_bool(sp.is_company),
                                ...
                                'create_date': make_aware(sp.create_date),
                                'title': sp.title,
                                ...
                                'partner_share': parse_bool(sp.partner_share),
                                'write_date': make_aware(sp.write_date),
                                ...
                                'autopost_bills': parse_bool(sp.autopost_bills),
                                ...
                                'trust': sp.trust,
                                'ignore_abnormal_invoice_date': parse_bool(sp.ignore_abnormal_invoice_date),
                                'ignore_abnormal_invoice_amount': parse_bool(sp.ignore_abnormal_invoice_amount),
                                ...
                                'calendar_last_notif_ack': make_aware(sp.calendar_last_notif_ack),
                                'website_id': sp.website_id,
                                'is_published': parse_bool(sp.is_published),
                                'date_localization': make_aware(sp.date_localization),
                                'buyer_id': sp.buyer_id,
                                'purchase_warn': sp.purchase_warn,
                                'property_purchase_currency_id': sp.property_purchase_currency_id,
                                'receipt_reminder_email': parse_bool(sp.receipt_reminder_email),
                                'reminder_date_before_receipt': sp.reminder_date_before_receipt,
                                'purchase_warn_msg': sp.purchase_warn_msg,
                                'is_converted_to_lead': parse_bool(sp.is_converted_to_lead),
